...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["csv_file"])) {
    ...
    $collection = $client->SorcerySetting->CampaignList;

       $file = $_FILES["csv_file"]["tmp_name"];
    if($file==""){ 
        header('Location: /campaign.php?message=1');
        exit();
    }
    else{
        ...
        foreach ($documents as $document1) {
            if(empty($document1['CampaignID'])){
                header('Location: /campaign.php?message=2');
                exit();
                break;
            }
                $filter = ['CampaignID' => $document1['CampaignID']];
                $update = ['$set' => $document1];
                $options = ['multi' => false, 'upsert' => false];
                $result = $collection->updateMany($filter, $update, $options);
                }
                header('Location: /campaign.php?message=3');
            exit();
}
}
